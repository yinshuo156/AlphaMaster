...
class AShareDataLoader:
    ...
    def __init__(self, data_path: str = "data/a_share"):
        ...
        for file_path in csv_files:
            try:
                ...
                for symbol in df['symbol'].unique():
                    ...
                    self.stock_data[symbol] = symbol_data
                    print(f"加载A股数据: {symbol}, 数据量: {len(symbol_data)}")
...
class AShareAlphaGFNGenerator:
    """基于GFlowNet的A股Alpha因子生成器 - 超优化版"""
    
    def __init__(self, data_loader: AShareDataLoader):
        self.data_loader = data_loader
        self.window_size = 5
        self.normalizer = UltraFactorNormalizer()
        
    def generate_factors(self) -> Dict[str, pd.DataFrame]:
        """生成GFlowNet风格的A股alpha因子 - 超优化版"""
        factors = {}
        ...
        factors['GFN_Momentum'] = momentum_factor
        ...
        factors['GFN_Volatility'] = volatility_factor
        ...
        factors['GFN_VolumeCorr'] = volume_corr_factor
        ...
        factors['GFN_LogPrice'] = log_returns
        ...
        factors['GFN_Composite'] = composite_factor
        
        return factors

class AShareAlphaAgentGenerator:
    """基于AlphaAgent的A股因子生成器 - 超优化版"""
    ...
    def generate_factors(self) -> Dict[str, pd.DataFrame]:
        ...
        factors['Agent_MACD'] = macd_factor
        ...
        factors['Agent_Bollinger'] = bollinger_factor
        ...
        factors['Agent_VWAP'] = vwap_factor
        ...
        factors['Agent_MomentumReversal'] = momentum_reversal_factor
        
        return factors

class AShareAlphaGenGenerator:
    """基于AlphaGen的A股因子生成器 - 超优化版，深度修复波动率回归因子"""
    ...
    def generate_factors(self) -> Dict[str, pd.DataFrame]:
        ...
        factors['Gen_RelativeStrength'] = relative_strength_factor
        ...
        factors['Gen_VolumeAnomaly'] = volume_anomaly_factor
        ...
        factors['Gen_PriceAcceleration'] = price_acceleration_factor
        ...
        # 不再进行额外标准化，直接使用
        factors['Gen_VolatilityReversion'] = volatility_reversion_factor
        ...
        factors['Gen_MultiMomentum'] = multi_momentum_factor
        
        return factors

class AShareAlphaMinerGenerator:
    """基于AlphaMiner的A股因子生成器 - 超优化版"""
    ...
    def generate_factors(self) -> Dict[str, pd.DataFrame]:
        ...
        factors['Miner_SMACross'] = sma_cross_factor
        ...
        factors['Miner_WilliamsR'] = williams_r_factor
        ...
        factors['Miner_Stochastic'] = stochastic_factor
        ...
        factors['Miner_CCI'] = cci_factor
        
        return factors

class AShareGeneticAlphaGenerator:
    """基于Genetic-Alpha的A股因子生成器 - 超优化版"""
    ...
    def generate_factors(self) -> Dict[str, pd.DataFrame]:
        ...
        factors['Genetic_Composite'] = genetic_composite_factor
        ...
        factors['Genetic_AdaptiveMA'] = adaptive_ma_factor
        ...
        factors['Genetic_EvolutionaryMomentum'] = evolutionary_momentum_factor
        ...
        factors['Genetic_Multivariate'] = multivariate_factor
        ...
        factors['Genetic_Optimal'] = optimal_factor
...
class AShareAlphaFactorGenerator:
    """A股Alpha因子生成器主类 - 超优化版"""
    
    def __init__(self, data_path: str = "data/a_share"):
        self.data_loader = AShareDataLoader(data_path)
        self.gfn_generator = AShareAlphaGFNGenerator(self.data_loader)
        self.agent_generator = AShareAlphaAgentGenerator(self.data_loader)
        self.gen_generator = AShareAlphaGenGenerator(self.data_loader)
        self.miner_generator = AShareAlphaMinerGenerator(self.data_loader)
        self.genetic_generator = AShareGeneticAlphaGenerator(self.data_loader)
    
    def generate_all_factors(self) -> Dict[str, pd.DataFrame]:
        """生成所有因子 - 超优化版"""
        print("开始生成A股Alpha因子 - 超优化版...")
    ...
    def save_factors(self, factors: Dict[str, pd.DataFrame], output_file: str = "alpha_pool/a_share_alpha_factors_ultra_optimized.csv"):
        """保存因子数据"""
        ...
        for factor_name, factor_df in factors.items():
            ...
            for date in factor_df.index:
                for stock in factor_df.columns:
                    value = factor_df.loc[date, stock]
...
def main():
    """主函数"""
    print("=" * 60)
    print("A股Alpha因子生成器 - 超优化版")
    print("深度优化所有问题因子，确保数值稳定性和质量")
    print("=" * 60)
    
    # 创建生成器
    generator = AShareAlphaFactorGenerator()
