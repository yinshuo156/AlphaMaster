    ...
    def load_all_data(self):
        ...
        for file_path in csv_files:
            try:
                ...
                # 处理成交量数据（移除逗号和引号）
                df['volume'] = df['volume (10K shares)'].astype(str).str.replace(',', '').str.replace('"', '').astype(float)
                ...
                for symbol in df['symbol'].unique():
                    ...
                    print(f"加载美股数据: {symbol}, 数据量: {len(symbol_data)}")
...
class USAlphaGFNGenerator:
    """基于GFlowNet的美股Alpha因子生成器 - 超优化版"""
    
    def __init__(self, data_loader: USStockDataLoader):
        self.data_loader = data_loader
        self.window_size = 5
        self.normalizer = UltraFactorNormalizer()
        
    def generate_factors(self) -> Dict[str, pd.DataFrame]:
        """生成GFlowNet风格的美股alpha因子 - 超优化版"""
        factors = {}
...
class USAlphaAgentGenerator:
    """基于AlphaAgent的美股因子生成器 - 超优化版"""
...
class USAlphaGenGenerator:
    """基于AlphaGen的美股因子生成器 - 超优化版，深度修复波动率回归因子"""
...
class USAlphaMinerGenerator:
    """基于AlphaMiner的美股因子生成器 - 超优化版"""
...
class USGeneticAlphaGenerator:
    """基于Genetic-Alpha的美股因子生成器 - 超优化版"""
...
class USAlphaFactorGenerator:
    """美股Alpha因子生成器主类 - 超优化版"""
    
    def __init__(self, data_path: str = "data/us"):
        self.data_loader = USStockDataLoader(data_path)
        self.gfn_generator = USAlphaGFNGenerator(self.data_loader)
        self.agent_generator = USAlphaAgentGenerator(self.data_loader)
        self.gen_generator = USAlphaGenGenerator(self.data_loader)
        self.miner_generator = USAlphaMinerGenerator(self.data_loader)
        self.genetic_generator = USGeneticAlphaGenerator(self.data_loader)
    
    def generate_all_factors(self) -> Dict[str, pd.DataFrame]:
        """生成所有因子 - 超优化版"""
        print("开始生成美股Alpha因子 - 超优化版...")
    ...
    def save_factors(self, factors: Dict[str, pd.DataFrame], output_file: str = "alpha_pool/us_alpha_factors_ultra_optimized.csv"):
        """保存因子数据"""
...
def main():
    """主函数"""
    print("=" * 60)
    print("美股Alpha因子生成器 - 超优化版")
    print("深度优化所有问题因子，确保数值稳定性和质量")
    print("=" * 60)
    
    # 创建生成器
    generator = USAlphaFactorGenerator()
